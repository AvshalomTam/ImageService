...
namespace ImageService.Controller.Handlers
{
    ...
    public class DirectoyHandler : IDirectoryHandler
    {
        ...
        {
            //first need to check if the command is mentioned to me (or to everyone)
        ...
        {
            //stop the listening and send message "stopped listening to ..."
            foreach (FileSystemWatcher watcher in m_dirWatchers)
            {
                watcher.EnableRaisingEvents = false;
                watcher.Dispose();
            }
            DirectoryClose?.Invoke(this, new DirectoryCloseEventArgs(m_path, $"Handler closed : {m_path}"));
        }
    }
}
